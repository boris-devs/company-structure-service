...
class DepartmentsService:
	...
	async def _commit(self, operation: str, **log_context) -> None:
		try:
			...
			logger.exception("%s failed; transaction rolled back", operation, extra=log_context)
			raise
	...
	async def create_department(self, name: str, parent_id: int | None = None):
		...
		if parent_id is not None:
			...
			if not parent_exists:
				logger.warning("Department creation rejected: parent not found", extra={"parent_department_id": parent_id})
				raise HTTPException(
		...
		try:
			...
			await self.depart_repo.db.rollback()
			logger.exception("Department creation failed due to integrity error", extra={"parent_department_id": parent_id})
			raise HTTPException(
				status_code=status.HTTP_409_CONFLICT,
	...
	async def create_employeer_in_department(self, department_id: int, data: CreateEmployeeInDepartmentRequestSchema):
		logger.info("Creating employee in department", extra={"department_id": department_id})
		exist_department = await self.depart_repo.get_department_by_id(department_id)
		if not exist_department:
			logger.warning("Employee creation rejected: department not found", extra={"department_id": department_id})
			raise HTTPException(
		...
		try:
			...
			await self.depart_repo.db.rollback()
			logger.exception("Employee creation failed due to integrity error", extra={"department_id": department_id})
			raise HTTPException(
				status_code=status.HTTP_409_CONFLICT,
	...
	async def full_info_department(self, department_id: int, depth: int = 1, include_employees: bool = True):
		...
		if not exist_department:
			logger.warning("Department details rejected: department not found", extra={"department_id": department_id})
	...
	async def reassign_department(self, department_id: int, data: ReassignDepartmentRequestSchema):
		...
		if data.parent_id == department_id:
			logger.warning("Department reassignment rejected: target is self", extra={"department_id": department_id})
			raise HTTPException(
				status_code=status.HTTP_400_BAD_REQUEST,
		...
		exist_department = await self.depart_repo.get_department_by_id(department_id)
		if not exist_department:
			logger.warning("Department reassignment rejected: department not found", extra={"department_id": department_id})
			raise HTTPException(
				status_code=status.HTTP_404_NOT_FOUND,
		...
		if data.parent_id is not None:
			...
			if not target_parent:
				logger.warning(
					...
					extra={"department_id": department_id, "target_parent_department_id": data.parent_id},
				)
				raise HTTPException(
					status_code=status.HTTP_404_NOT_FOUND,
	...
	async def delete_department(self, department_id: int, mode: str, reassign_to_id: int | None):
		logger.info(
			"Deleting department",
			extra={"department_id": department_id, "mode": mode, "reassign_to_department_id": reassign_to_id},
		)
		target_dept = await self.depart_repo.get_department_by_id(department_id)
		if not target_dept:
			logger.warning("Department deletion rejected: department not found", extra={"department_id": department_id})
			raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Department not found.")
			...
			if mode == "cascade":
				...
				if not new_home:
					logger.warning(
						...
						extra={"department_id": department_id, "reassign_to_department_id": reassign_to_id},
					)
					raise HTTPException(
						status_code=status.HTTP_404_NOT_FOUND,
